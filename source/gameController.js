...
function start(req, res) {
    if (openIds.length === 0) return res.json({
        success: false,
        error: 'No IDs available'
    });
    let indexOfGameId = req.body.gameId ? openIds.indexOf(parseInt(req.body.gameId)) : 0;
    if (indexOfGameId === -1 && timeouts[req.body.gameId] > new Date()) return res.json({
        success: false,
...
function join(req, res) {
    ...
    if (!games[gameId]) return res.json({
        success: false,
        error: 'Game not found.'
    });
    if (games[gameId].users[username]) return res.json({
        success: false,
        error: 'Name unavailable.'
    });
    games[gameId].users[username] = {
        score: 0,
        round: 0
    };
    updateTimeout(gameId);
...
function ask(req, res) {
    ...
    const remainingTime = games[gameId].roundId ? games[gameId].rounds[games[gameId].roundId].end.getTime() - new Date().getTime() : -postQuestionDuration;
    if (remainingTime > -postQuestionDuration) {
        const timeToClearQueue = remainingTime + postQuestionDuration + games[gameId].queuedRounds * (questionDuration + postQuestionDuration);
        setTimeout(() => {
        ...
        return res.json({
            success: true,
            round: games[gameId].roundId + games[gameId].queuedRounds,
            queuePosition: games[gameId].queuedRounds,
            timeToResult: timeToClearQueue + questionDuration
        });
    }
...
function generateNewQuestion(gameId) {
    ...
    if (question.type === 'choice') {
        games[gameId].rounds[games[gameId].roundId] = {
            question: question.id,
            participants: 0,
            correct: 0,
            a: 0,
...
function getQuestion(req, res) {
    ...
    if (!round || round.end < new Date()) return res.json({
        success: false,
...
function answer(req, res) {
    ...
    if (!games[gameId].rounds[roundId]) return res.json({
        success: false,
    ...
    if (round.end < new Date()) return res.json({
        success: false,
        error: 'Round over.'
    });
    const question = quizController.getQuestion(round.question);
    if (games[gameId].users[username].round >= roundId) return res.json({
        success: false,
        error: 'User has already answered.'
    });
    if (question.type === 'choice' && !['a', 'b', 'c', 'd'].includes(answer)) return res.json({
        success: false,
        error: 'Wrong answer format.'
    });
    if (question.type === 'estimate' && isNaN(answer)) return res.json({
        success: false,
        error: 'Answer was not a number.'
    });
    if (question.type === 'order' && (answer.length != 4 || !answer.includes('a') || !answer.includes('b') || !answer.includes('c') || !answer.includes('d'))) return res.json({
        success: false,
        error: 'Wrong answer format.'
    });
