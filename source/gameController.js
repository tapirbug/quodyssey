...
router.post('/ask', (req, res, next) => {
    ...
    if (type == 'choice') {
        ...
        games[gameId].rounds[games[gameId].roundId] = { 'question': randomizedQuestion, 'participants': 0, 'correct': 0, 'end': date };
    }
    return res.json({ 'success': true, 'round': games[gameId].roundId, 'question': randomizedQuestion });
});

router.get('/getQ/:gameId', (req, res, next) => {
...
router.get('/resultQ/:gameId/:roundId', (req, res, next) => {
    let gameId = req.params.gameId;
...
router.get('/resultA/:gameId/:roundId', (req, res, next) => {
    let gameId = req.params.gameId;
    let roundId = req.params.roundId;
    if (games[gameId] == undefined) return res.json({ 'success': false, 'error': 'Game not found.' });
    if (games[gameId].rounds[roundId] == undefined) return res.json({ 'success': false, 'error': 'Round not found.' });
...
router.get('/scoreboard/:gameId', (req, res, next) => {
    let gameId = req.params.gameId;
    if (games[gameId] == undefined) return res.json({ 'success': false, 'error': 'Game not found.' });
